...
class CentralAgent(object):
    ...
    def __init__(self, envt: Environment, is_epsilon_greedy: bool = False):
        ...
        self.envt = envt
        self._choose = (
            self._epsilon_greedy if is_epsilon_greedy else self._additive_noise
        )

    def choose_actions(
        self,
        agent_action_choices: List[List[Tuple[Action, float]]],
    ...
    def _additive_noise(
        ...
        final_actions = self._choose_actions_ILP(
            agent_action_choices, get_noise=get_noise
        )
    ...
    def _choose_actions_ILP(
        ...
        for agent_idx in range(len(agent_action_choices)):
            ...
            model.add_constraint(
                model.sum(variable for variable, _ in agent_specific_variables) == 1
            )
        ...
        for request in requests:
            ...
            model.add_constraint(
                model.sum(
                    variable
                    for action_dict in relevent_action_dicts
                    for variable, _ in action_dict.values()
                )
                <= 1
            )
        ...
        # Create Objective
        score = model.sum(
            (value + get_noise(variable)) * variable
            for action_dict in decision_variables.values()
            for (variable, value) in action_dict.values()
        )
    ...
    def _choose_actions_random(
        self, agent_action_choices: List[List[Tuple[Action, float]]]
    ) -> List[Tuple[Action, float]]:
        final_actions: List[Optional[Tuple[Action, float]]] = [None] * len(
        ...
        for agent_idx in order:
            ...
            for action_idx, (action, _) in enumerate(agent_action_choices[agent_idx]):
                is_not_consumed = [
                    (request in consumed_requests) for request in action.requests
                ]
            ...
            # Update infeasible action information
