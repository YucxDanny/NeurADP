...
class Path(object):
    ...
    def visit_next_location(self, current_time: float):
        if not self.is_empty():
            ...
            if next_node.is_dropoff:
                ...
                self.total_delay += (
                    next_node.expected_visit_time
                    - next_request.request.dropoff_deadline
                )
                ...
                if not self.requests[relevant_request_id].is_dummy:
                    assert (
                        self.requests[relevant_request_id].request.pickup_deadline
                        >= current_time
                    )
    ...
    def get_info(self, node: "PathNode") -> Tuple[int, float]:
        ...
        if node.is_dropoff:
            location = relevant_request.request.dropoff
            deadline = relevant_request.request.dropoff_deadline
        else:
            location = relevant_request.request.pickup
...
class RequestInfo(object):
    ...
    def __init__(self, request: Request, has_been_picked_up: bool, is_dummy: bool):
