...
class Oracle(object):
    ...
    The Oracle is used to generate the feasible actions for each agent.
    """
    ...
    def get_feasible_actions(
        ...
        for requests_for_agent, agent in zip(requests_for_each_agent, agents):
            ...
            nodes_to_expand = [
                (trip_priority(trip), trip_idx + 1)
                for trip_idx, trip in enumerate(trips[1:])
            ]
            heapq.heapify(nodes_to_expand)
            while (
                len(nodes_to_expand) > 0
                and num_is_feasible_calls < MAX_IS_FEASIBLE_CALLS
            ):
                ...
                for trip_size_1_idx in trips_size_1:
                    ...
                    if new_action not in tested_actions:
                        ...
                        assert (
                            action.new_path is not None
                        )  # Make sure no invalid actions slipped past
                        new_path = self.get_new_path(
                            agent, action.new_path, new_request
                        )
                        ...
                        if new_path is not None:
                            ...
                            heapq.heappush(
                                nodes_to_expand,
                                (trip_priority(new_action), len(trips) - 1),
                            )
    ...
    def get_new_path_complete_search(self, agent: LearningAgent, path: Path) -> Path:
        ...
        path.total_delay = (
            Path.NOT_APPLICABLE
        )  # indicates that we haven't built a new path yet
        ...
        while True:
            ...
            if current_index < len(possible_next_nodes):
                ...
                if not current_request_order:
                    ...
                    current_time = (
                        self.envt.current_time + agent.position.time_to_next_location
                    )
            ...
            # Else, check if this path has been completed
                ...
                if path.is_complete(current_request_order):
                    ...
                    if (
                        previous_lowest_delay == Path.NOT_APPLICABLE
                        or current_remaining_delay > previous_lowest_delay
                    ):
                        # Save, if it is
            ...
            if stepBack:
                if stack:
                    possible_next_nodes, current_index, current_remaining_delay = (
                        stack.pop()
                    )
    ...
    def get_new_path_insertion(self, agent: LearningAgent, path: Path) -> Path:
        ...
        relevant_request_id = (
            len(path.requests) - 1
        )  # Assume the new request is the last one
    ...
    def _insert_path(
        ...
        if min_delay_idx != Path.NOT_APPLICABLE:
            ...
            self._insert_pathnode(
                agent,
                path,
                new_node,
                min_delay_idx,
                min_individual_delay,
                max_insert_idx,
            )
            # Update total_delay
    ...
    def _can_insert_node(
        ...
        prev_location, prev_deadline, visit_time, current_capacity = (
            self._get_node_info(agent, path, insert_idx - 1)
        )
    ...
    def get_path_insertion_2(self, agent: LearningAgent, path: Path) -> Path:
        ...
        for dropoff_idx in range(len(path.request_order), -1, -1):
            dropoff_delay, new_node_delay, dropoff_future_delay = self._can_insert_node(
                agent,
                path,
                dropoff["new_node"],
                dropoff_idx,
                dropoff["min_future_delay"],
            )
            ...
            if dropoff_delay != Path.NOT_APPLICABLE:
                ...
                pickup["min_future_delay"] = min(
                    dropoff["min_future_delay"] - dropoff_delay, new_node_delay
                )
                pickup["num_dropoffs_after"] = dropoff["num_dropoffs_after"] + 1

                for pickup_idx in range(dropoff_idx, -1, -1):
                    pickup_delay, _, pickup["min_future_delay"] = self._can_insert_node(
                        agent,
                        path,
                        pickup["new_node"],
                        pickup_idx,
                        pickup["min_future_delay"],
                    )
                    ...
                    if pickup_delay != Path.NOT_APPLICABLE:
                        ...
                        total_delay -= (
                            dropoff_delay * dropoff["num_dropoffs_after"]
                        )  # subtracting delay due to pickup
                        total_delay -= (
                            pickup_delay * pickup["num_dropoffs_after"]
                        )  # subtracting delay due to dropoff
                        total_delay += (
                            new_node_delay  # adding available delay for the new request
                        )
        ...
        if max_total_delay != Path.NOT_APPLICABLE:
            ...
            self._insert_pathnode(
                ...
                path,
                dropoff["new_node"],
                dropoff["min_delay_idx"],
                dropoff["min_individual_delay"],
            )
            # Pickup
            self._insert_pathnode(
                agent,
                path,
                pickup["new_node"],
                pickup["min_delay_idx"],
                pickup["min_individual_delay"],
                dropoff["min_delay_idx"],
            )
            # Update total_delay
            path.total_delay = max_total_delay
