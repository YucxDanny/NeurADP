...
from random import (
    ...
    random,
    choice,
)  # choice function randomly select an element from a non-empty sequence and return it
...
class Environment(metaclass=ABCMeta):
    ...
    def __init__(
        ...
        self.recent_request_history: Deque[Request] = deque(
            maxlen=self.REQUEST_HISTORY_SIZE
        )
    ...
    def simulate_motion(
        ...
        if rebalance and agents_to_rebalance:
            rebalancing_targets = self._get_rebalance_targets(
                [agent for agent, _ in agents_to_rebalance]
            )
            ...
            for idx, target in enumerate(rebalancing_targets):
                ...
                agent.path.request_order.append(
                    PathNode(False, 0)
                )  # adds pickup location to 'to-visit' list
            ...
            if time_remaining >= 0:
                ...
                if agent.position.next_location == agent.path.get_next_location():
                    agent.path.visit_next_location(
                        self.current_time + self.EPOCH_LENGTH - time_remaining
                    )

                # Go to the next location in the path, if it exists
                if not agent.path.is_empty():
                    next_location = self.get_next_location(
                        agent.position.next_location, agent.path.get_next_location()
                    )
                    agent.position.time_to_next_location = self.get_travel_time(
    ...
    def _get_rebalance_targets(self, agents: List[LearningAgent]) -> List[Request]:
        ...
        assignments = model.continuous_var_matrix(
            range(len(agents)), range(len(possible_targets)), name="assignments"
        )
        ...
        num_fractional_targets = len(agents) - (
            int(len(agents) / num_targets) * num_targets
        )
        for target_id in range(len(possible_targets)):
            num_agents_to_target = int(len(agents) / num_targets) + (
                1 if target_id < num_fractional_targets else 0
            )
            model.add_constraint(
                model.sum(
                    assignments[agent_id, target_id] for agent_id in range(len(agents))
                )
                == num_agents_to_target
            )
    ...
    def get_reward(self, action: Action) -> float:
        ...
        The reward is defined as the sum of all request values.
...
class NYEnvironment(Environment):
    ...
    def initialise_environment(self):
        ...
        assert (
            ...
            or (self.EPOCH_LENGTH == 30)
            or (self.EPOCH_LENGTH == 10)
        )
        self.DATA_FILE_PREFIX: str = "{}files_{}sec/test_flow_5000_".format(
            self.DATA_DIR, int(self.EPOCH_LENGTH)
        )

    def get_request_batch(
        self, day: int = 2, downsample: float = 1
    ) -> Generator[List[Request], None, None]:
        ...
        def is_in_time_range(current_time):
            ...
            return (
                True
                if (
                    current_hour >= self.START_EPOCH / 3600
                    and current_hour < self.STOP_EPOCH / 3600
                )
                else False
            )
        ...
        with open(self.DATA_FILE_PREFIX + str(day) + ".txt", "r") as data_file:
            ...
            for line in data_file.readlines():
                ...
                if is_new_epoch is not None:
                    ...
                    )  # Make sure there's nothing funky going on with the formatting
                    ...
                    for _ in range(num_requests):
                        ...
                        if rand_num <= downsample:
                            ...
                            if (
                                source not in self.ignored_zones
                                and destination not in self.ignored_zones
    ...
    def has_valid_path(self, agent: LearningAgent) -> bool:
        ...
        if (
            agent.path.current_capacity < 0
            or agent.path.current_capacity > self.MAX_CAPACITY
        ):
        ...
        for node_idx, node in enumerate(agent.path.request_order):
            ...
            if current_time + travel_time > deadline:
                return invalid_path_trace(
                    "Does not meet deadline at node {}".format(node_idx)
                )
            ...
            if node.expected_visit_time != current_time:
                invalid_path_trace(
                    "(Ignored) Visit time incorrect at node {}".format(node_idx)
                )
            ...
            if current_capacity > self.MAX_CAPACITY:
                return invalid_path_trace(
                    "Exceeds MAX_CAPACITY at node {}".format(node_idx)
                )
            ...
            if node.current_capacity != next_capacity:
                invalid_path_trace(
                    "(Ignored) Capacity incorrect at node {}".format(node_idx)
                )
