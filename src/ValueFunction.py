...
from tensorflow.summary import create_file_writer, scalar  # type: ignore
# from tensorflow.core.framework import summary_pb2  # type: ignore
...
class ValueFunction(ABC):
    ...
    def add_to_logs(self, tag: str, value: float, step: int) -> None:
        ...
        with self.summary_writer.as_default():
            scalar(name=tag, data=value, step=step)
...
class NeuralNetworkBased(ValueFunction):
    ...
    def _soft_update_function(self, target_model: Model, source_model: Model) -> keras_function:
        ...
        print("Length of target weights:", len(target_weights))
        print("Length of source weights:", len(source_weights))
    ...
    def _format_input_batch(self, all_agents_post_actions: List[List[LearningAgent]], current_time: float, num_requests: int) -> Dict[str, Any]:
        ...
        for agent_post_actions in all_agents_post_actions:
            ...
            for agent in agent_post_actions:
                ...
                num_requests_input.append(num_requests) # num_requests_input
            ...
            input["delay_input"].append(delay_input) # List[List[ndarray(max_cap*2+1)]]
            input["path_location_input"].append(path_location_input) # List[List[ndarray(max_cap*2+1) (num_actions)](num_agents)]
