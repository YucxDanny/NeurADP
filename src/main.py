...
# from ValueFunction import PathBasedNN, NeuralNetworkBased
from ValueFunTorch import PathBasedNN
...
def run_epoch(
    ...
    """
    Runs one epoch of the simulation.

    Args:
    ------
        - envt: NYEnvironment object
        - oracle: Oracle object, used to generate the feasible actions for each agent
        - central_agent: CentralAgent object
        - value_function: PathBasedNN object
        - DAY: (int), the day to run
        - is_training: (bool), whether the simulation is training or not
        - agents_predefined: (List[LearningAgent]), list of agents to run the simulation with
        - TRAINING_FREQUENCY: (int) how often to train the value function

    Returns:
    ------
        - total_value_generated: (int) the total value generated by the simulation
    """
    ...
    total_value_generated = 0  # total_requests_served
    ...
    while True:
        ...
        # for agent_idx, feasible_actions in enumerate(feasible_actions_all_agents):
        #     print(agent_idx)
        ...
        scored_final_actions: List[Tuple[Action, float]] = central_agent.choose_actions(
        ...
        if is_training:
            ...
            if (
                int(envt.current_time) / int(envt.EPOCH_LENGTH)
        ...
        value_function.add_to_logs(
            "rewards_day_{}".format(envt.num_days_trained),
            sum(rewards),
            int(envt.current_time),
        )

        avg_capacity = (
            sum([agent.path.current_capacity for agent in agents]) / envt.NUM_AGENTS
        )

        value_function.add_to_logs(
            "avg_capacity_day_{}".format(envt.num_days_trained),
            avg_capacity,
            int(envt.current_time),
        )
...
if __name__ == "__main__":
    ...
    END_HOUR: int = 24
    NUM_EPOCHS: int = 3
    TRAINING_DAYS: List[int] = [1]
    VALID_DAYS: List[int] = [2]
    TEST_DAYS: List[int] = [2]
    VALID_FREQ: int = 4
    SAVE_FREQ: int = 1
    LOG_DIR: str = "../logs/{}agent_{}capacity_{}delay_{}interval/".format(
        args.numagents, args.capacity, args.pickupdelay, args.decisioninterval
    )
    ...
    envt = NYEnvironment(
        ...
        EPOCH_LENGTH=args.decisioninterval,
    )

    oracle = Oracle(envt)
    central_agent = CentralAgent(envt)

    value_function = PathBasedNN(
        envt, log_dir=LOG_DIR, load_model_loc=args.modellocation
    )
    ...
    for epoch_id in range(NUM_EPOCHS):
        for day in TRAINING_DAYS:
            total_requests_served = run_epoch(
                envt, oracle, central_agent, value_function, day, is_training=True
            )
            print("\nDAY: {}, Requests: {}\n\n".format(day, total_requests_served))
            value_function.add_to_logs(
                "requests_served", total_requests_served, envt.num_days_trained
            )
            ...
            if envt.num_days_trained % VALID_FREQ == VALID_FREQ - 1:
                ...
                value_function.add_to_logs(
                    "validation_score", test_score, envt.num_days_trained
                )

                # TODO: Save results better
                if isinstance(value_function, PathBasedNN):
                    if test_score > max_test_score or (
                        envt.num_days_trained % SAVE_FREQ
                    ) == (SAVE_FREQ - 1):
                        value_function.model.save(
                            "../models/{}_{}agent_{}capacity_{}delay_{}interval_{}_{}.h5".format(
                                type(value_function).__name__,
                                args.numagents,
                                args.capacity,
                                args.pickupdelay,
                                args.decisioninterval,
                                envt.num_days_trained,
                                test_score,
                            )
                        )
                        max_test_score = (
    ...
    for day in TEST_DAYS:
        ...
        print("\n(TEST) DAY: {}, Requests: {}\n\n".format(day, total_requests_served))
        value_function.add_to_logs(
            "test_requests_served", total_requests_served, envt.num_days_trained
        )
